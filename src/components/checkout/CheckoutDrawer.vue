      ...
      <div class="checkout-overlay__container">
        ...
        >
          ...
          <ul class="checkout-shell__progress" role="list">
            <li
              class="checkout-shell__progress-step"
              :class="{
          ...
          <div v-if="hasCartItems" class="checkout-shell__content">
            <div class="checkout-shell__grid">
                ...
                <section class="checkout-cart">
                  <header class="checkout-card__header">
                    <div>
                      ...
                      <span
                        >{{ cartItemCount }}
                        {{ cartItemCount == 1 ? `item` : `items` }}</span
                      >
                    </div>
                  </header>
                  <ul class="checkout-cart__list">
                    <li
                      ...
                      :class="[
                        'checkout-cart__item',
                        { active: item.id === activeCartItemId },
                      ]"
                    >
                      <button
                        type="button"
                        class="checkout-cart__select"
                        @click="selectCartItem(item)"
                        :aria-pressed="item.id === activeCartItemId"
                      >
                        <span
                          class="checkout-cart__thumb"
                          :class="{ 'has-image': Boolean(item.previewImage) }"
                        >
                          <img
                    ...
                    <li
                      :key="'newitem'"
                      class="checkout-cart__item checkout-cart__item--new"
                    >
                      <button
                        type="button"
                        class="checkout-cart__add"
                        aria-label="Add a new item"
                        @click="handleAddNewItem"
                      >
                        <span
                          class="checkout-cart__add-icon"
                          aria-hidden="true"
                        >
                          <svg
                        ...
                        <span class="checkout-cart__add-label"
                  ...
                  <div class="checkout-preview__viewer">
                    <div class="checkout-preview__controls">
                      <button
                        ...
                        :key="view"
                        type="button"
                        class="checkout-preview__control"
                        :class="{ active: previewView === view }"
                        :disabled="!previewAvailability[view]"
                        :aria-pressed="previewView === view"
                        @click="setPreviewView(view)"
                      >
                        {{ viewLabels[view] }}
                      </button>
                      <button
                        type="button"
                        class="checkout-preview__control"
                        :class="{ active: showDesignOnly }"
                        :disabled="!hasDesignOnlyAny"
                        :aria-pressed="showDesignOnly"
                    ...
                    <div
                      class="checkout-preview__frame"
                      :class="{ 'has-image': Boolean(activePreviewSrc) }"
                    >
                      <img
                        v-if="activePreviewSrc"
                        :src="activePreviewSrc"
                        :alt="`${viewLabels[previewView]} view of ${
                          activeCartItem?.product?.name ?? 'selected item'
                        }`"
                      />
                ...
                >
                  <div class="checkout-form__grid">
                    ...
                    <label class="checkout-form__notes">
                      ...
                      <textarea
                        rows="4"
                        v-model="notesField"
                        placeholder="Share artwork details, deadlines, and special requests"
                      ></textarea>
                    </label>
                  </div>
                  <div class="checkout-summary">
                    <div class="checkout-summary__section">
                      <div
                        v-for="detail in cartPricingDetails"
                        :key="detail.item.id"
                      ...
                      >
                        <div class="summary-item__info">
                          <span class="summary-item__title">
                            ...
                            <template v-if="detail.item.quantity > 1">
                              ({{ detail.item.quantity }}x)</template
                            >
                          </span>
                          <ul class="summary-item__charges">
                            <li>
                              ...
                              <strong>{{
                                detail.formatted.basePerUnit ?? "—"
                              }}</strong>
                            </li>
                            <li
                              v-for="(charge, idx) in detail.breakdown
                                .designCharges"
                              :key="idx"
                            >
                              <span>
                    ...
                    <div
                      v-if="cartPricingSummary.hasDiscount"
                      class="checkout-summary__section"
                    >
                      <div class="checkout-summary__discount">
                        <span> Quantity discount </span>
                        <strong
                          >-{{
                            cartPricingSummary.discountLabel ?? "—"
                          }}</strong
                        >
                      </div>
                    </div>
                    <div class="checkout-summary__section summary-taxes">
                      <div class="checkout-summary__item summary-tax">
                        <span>Tax: </span>
                        <strong>{{
                          cartPricingSummary.taxLabel ?? "—"
                        }}</strong>
                      </div>
                      <div class="checkout-summary__item summary-tax">
                        <span>Processing fee:</span>
                        <strong>{{
                          cartPricingSummary.stripeFeeLabel ?? "—"
                        }}</strong>
                      </div>
                    </div>
                    <div class="checkout-summary__total">
                      ...
                      <strong>{{
                        formatCurrency(
                          cartPricingSummary.finalTotal +
                            cartPricingSummary.taxAmount +
                            cartPricingSummary.stripeFees,
                          cartPricingSummary.currency ??
                            cartStore.firstCurrency ??
                            "USD"
                        ) ?? "—"
                      }}</strong>
              ...
              <section
                v-if="currentStep === 3"
                class="checkout-form-card-striper"
              >
                <header class="checkout-card__header">
                  <div>
                    <p v-if="requestStatus === 'success' && orderRecorded">
                      Checkout complete! We’ll follow up by email.
                    </p>
                    <p
                      v-else-if="requestStatus === 'success' && orderRecording"
                    >
                      Payment complete; finalizing your order…
                    </p>
                    <p v-else-if="requestStatus === 'processing'">
                ...
                <template v-if="requestStatus === 'success' && orderRecorded">
                  <div class="checkout-payment__success">
                    <p>
                      Thanks! Your payment was received and your order has been
                      saved. We’ll be in touch shortly with next steps.
                    </p>
                    <button
                      type="button"
                ...
                <template v-else>
                  <div v-if="!stripeConfigured" class="checkout-payment__error">
                    <p>
                      Payment is unavailable right now. Please contact us to
                      complete your order.
                    </p>
                  </div>
                  <div v-else class="checkout-payment">
                    <div
                      ...
                      :class="{ 'is-loading': !paymentElementReady }"
                    >
                      <div
                        ref="paymentElementRef"
                        class="checkout-payment__mount"
                      ></div>
                      <div
                        v-if="!paymentElementReady"
                        class="checkout-payment__loading"
                      >
        ...
        <footer v-if="hasCartItems" class="checkout-shell__footer">
          <div class="checkout-shell__footer-actions">
            ...
            <button
              ...
              type="button"
              class="checkout-form__submit"
              :disabled="!paymentFormReady || paymentProcessing"
...
)?.replace(/\/$/, ""); // base Supabase URL
const SUPABASE_ANON_KEY = (
  import.meta.env.VITE_SUPABASE_ANON_KEY as string | undefined
)?.trim(); // anon key (browser)
const ORDER_FUNCTION_URL =
  (import.meta.env.VITE_ORDER_FUNCTION_URL as string | undefined)?.trim() ||
  null;
const CHECKOUT_FUNCTION_URL =
  (import.meta.env.VITE_CHECKOUT_FUNCTION_URL as string | undefined)?.trim() ||
  null;
const STRIPE_PUBLISHABLE_KEY = (
  import.meta.env.VITE_STRIPE_PUBLISHABLE_KEY as string | undefined
)?.trim(); // Stripe client key
const ORDER_PREVIEWS_BUCKET =
  (
    import.meta.env.VITE_SUPABASE_ORDER_PREVIEWS_BUCKET as string | undefined
  )?.trim() || "order-previews"; // proof images
const ORDER_DESIGNS_BUCKET =
  (
    import.meta.env.VITE_SUPABASE_ORDER_DESIGNS_BUCKET as string | undefined
  )?.trim() || "order-design-assets"; // raw assets
const FIREBASE_STORAGE_BUCKET =
  (
    import.meta.env.VITE_FIREBASE_STORAGE_BUCKET as string | undefined
  )?.trim() || "firebase-storage";
const BYPASS_PAYMENTS =
  String(
    (import.meta.env.VITE_CHECKOUT_BYPASS_PAYMENTS as string | undefined) ?? ""
...
const stripeConfigured = computed(
  () => Boolean(STRIPE_PUBLISHABLE_KEY) || BYPASS_PAYMENTS
);
...
const PREVIEW_UPLOAD_SLOT_CONFIG: Record<
  PreviewUploadSlotKey,
  PreviewUploadSlotMeta
> = {
  "front-with-shirt": {
    suffix: "front",
    fileBase: "front",
...
const PREVIEW_UPLOAD_SLOTS = Object.keys(
  PREVIEW_UPLOAD_SLOT_CONFIG
) as PreviewUploadSlotKey[];
const WITH_SHIRT_UPLOAD_SLOTS = new Set<PreviewUploadSlotKey>([
  "front-with-shirt",
  "back-with-shirt",
]);
...
function buildUploadCandidates(
  item: CartItem,
  candidates: PreviewCandidateSet
) {
  const usingLive = checkoutStore.editingCartItemId === item.id;

  const designCacheRefs = item.designPreviewCacheRefs ?? {
    Front: null,
    Back: null,
  };
  const blankCacheRefs = item.blankDesignCacheRefs ?? {
    Front: null,
    Back: null,
  };
  ...
  const liveWithFront = usingLive
    ? checkoutStore.designPreviews?.Front ?? null
    : null;
  const liveWithBack = usingLive
    ? checkoutStore.designPreviews?.Back ?? null
    : null;
  const liveBlankFront = usingLive
    ? checkoutStore.blankDesignPreviews?.Front ?? null
    : null;
  const liveBlankBack = usingLive
    ? checkoutStore.blankDesignPreviews?.Back ?? null
    : null;
...
function viewHasDesign(
  state: SerializedDesignState | null,
  view: DesignViewName
): boolean {
...
function buildPreviewCandidates(item: CartItem): PreviewCandidateSet {
  const canvasSources = (item.canvasPreviews ?? {
    Front: null,
    Back: null,
  }) as Record<PreviewView, string | null>;
  const previewCacheRefs = item.designPreviewCacheRefs ?? {
    Front: null,
    Back: null,
  };
  const blankCacheRefs = item.blankDesignCacheRefs ?? {
  ...
  const itemDesignState = (item.designState ??
    null) as SerializedDesignState | null;
  ...
  const liveFrontWithShirt = usingLiveCheckoutPreviews
    ? normalizePreview(checkoutStore.designPreviews?.Front)
    : null;
  const liveBackWithShirt = usingLiveCheckoutPreviews
    ? normalizePreview(checkoutStore.designPreviews?.Back)
    : null;
  const liveFrontBlank = usingLiveCheckoutPreviews
    ? normalizePreview(checkoutStore.blankDesignPreviews?.Front)
    : null;
  const liveBackBlank = usingLiveCheckoutPreviews
    ? normalizePreview(checkoutStore.blankDesignPreviews?.Back)
    : null;
  ...
  logPreviewBuckets(
    "checkoutDrawer.buildPreviewCandidates",
    {
      frontWithShirt,
      backWithShirt,
      frontWithoutShirt,
...
const cartPricingSummary = computed(() => {
  ...
  const stripeFees =
    Math.round(
      ((finalTotal + taxAmount) * STRIPE_PERCENT + STRIPE_FIXED) * 100
    ) / 100;
  return {
    baseTotal,
    designTotal,
    ...
    discountLabel: discountTotal
      ? formatCurrency(Math.abs(discountTotal), currency)
      : null,
...
function designChargeLabel(charge: any): string {
  ...
  if (charge?.elementType === "composite") {
    const tier =
      charge?.category === "full" ? "Full coverage" : "Partial coverage";
  ...
  const index =
    typeof charge?.elementIndex === "number"
      ? ` #${charge.elementIndex + 1}`
      : "";
  const category =
    charge?.category === "full" ? "Full coverage" : "Partial coverage";
...
function designChargeMeta(charge: any): string | null {
  const parts: string[] = [];
  if (
    typeof charge?.widthInches === "number" &&
    typeof charge?.heightInches === "number"
  ) {
    parts.push(
      `${charge.widthInches.toFixed(1)}″ × ${charge.heightInches.toFixed(1)}″`
    );
  }
...
function designChargeItemsSummary(charge: any): string | null {
  ...
  const descriptors = charge.items.map((item: any) => {
    const typeLabelRaw = (item?.elementType ??
      item?.type ??
      "design") as string;
    ...
    const variant =
      typeof variantRaw === "string" && variantRaw.includes(":")
        ...
        : variantRaw;
    const label =
      item?.name && typeof item.name === "string" && item.name.trim().length
        ? item.name.trim()
        : variant || typeLabel;
    const width =
      typeof item?.widthInches === "number" ? item.widthInches : null;
    const height =
      typeof item?.heightInches === "number" ? item.heightInches : null;
    const size =
...
async function readBlobFromObjectUrl(url: string): Promise<Blob | null> {
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(
        "[Checkout] readBlobFromObjectUrl: fetch failed for blob URL",
        {
          url,
...
async function resolveSourceBlob(
  ...
  const normalizedSource =
    typeof source === "string" ? source.trim() : String(source ?? "").trim();
  ...
  if (normalizedSource.startsWith("blob:")) {
    ...
    try {
      ...
      console.warn(
        "[Checkout] blob: not resolvable after fetch",
        normalizedSource,
        error
      );
      return null;
  ...
  try {
    ...
    console.warn(
      "[Checkout] Failed to resolve source blob",
      { source: normalizedSource.slice(0, 120) },
      error
    );
    return null;
  }
...
async function applyTargetDpiIfNeeded(
  ...
  return {
    blob: stampedBlob,
    contentType: stampedBlob.type || contentType || blob.type || null,
  };
}
...
function logStorageUploadPlan(
  plan: StorageUploadPlanEntry[],
  context: { source: string; cacheKey: string }
) {
...
async function uploadImageSource(
  ...
  const targetDpi =
    typeof options?.targetDpi === "number"
      ? options.targetDpi
      : TARGET_DESIGN_DPI;
  ...
  if (!resolved) {
    console.warn("[Checkout] Unable to resolve source for upload", {
  ...
  const dpiAdjusted = await applyTargetDpiIfNeeded(
    resolved.blob,
  ...
  const fileName = baseName.endsWith(`.${fileExtension}`)
    ? baseName
    : `${baseName}.${fileExtension}`;
  const objectPath = [bucket, ...pathSegments, fileName]
    .filter(Boolean)
    .join("/");
...
async function uploadFromCandidateSources(
  ...
  if (!candidates.length) {
    console.warn(
      "[Checkout] uploadFromCandidateSources called with no candidates",
      {
        bucket,
        pathSegments,
        fallbackName,
  ...
  console.warn(
    "[Checkout] uploadFromCandidateSources exhausted all candidates without success",
    {
      fallbackName,
      attempts: attempt,
      bucket,
...
function collectDesignImageSources(
  designState: SerializedDesignState | null
): CollectedImageSource[] {
  ...
  for (const view of Object.values(designState.views)) {
    ...
    for (const image of view.images) {
      ...
      const original =
        typeof image?.originalSource === "string"
          ? image.originalSource.trim()
          : "";
      const cacheRef =
        typeof image?.assetCacheRef === "string"
          ? image.assetCacheRef.trim()
          : "";
...
function buildPreviewUploadRequests(
  ...
  return slots.map((slot) => {
    ...
    const required =
      meta.enforce ||
      (meta.enforceWhenCandidates ? candidates.length > 0 : false);
    return {
      slot,
...
function logPreviewUploadManifest(
  cartItemId: string,
  requests: PreviewUploadRequest[]
) {
  try {
    ...
    console.warn(
      "[PreviewUpload] Failed to log manifest",
      { cartItemId },
      error
    );
  }
...
async function executePreviewUploadRequests(
  requests: PreviewUploadRequest[]
): Promise<PreviewUploadResultMap> {
  ...
  for (const request of requests) {
    ...
    if (!request.candidates.length) {
      ...
      try {
        ...
        console.error(
          "[PreviewUpload] Upload threw",
          { slot: request.slot, cartItemId: request.cartItemId },
          error
        );
      }
    }
...
function logPreviewUploadResults(
  cartItemId: string,
  results: PreviewUploadResultMap
) {
  try {
    ...
    console.warn(
      "[PreviewUpload] Failed to log results",
      { cartItemId },
      error
    );
  }
...
function mapPreviewUploadUrls(
  results: PreviewUploadResultMap
): ExportPreviewUploads {
  return {
    frontWithShirt: previewSlotEnabled("front-with-shirt")
      ? results["front-with-shirt"]?.url ?? null
      : null,
    frontWithoutShirt: results["front-without-shirt"]?.url ?? null,
    backWithShirt: previewSlotEnabled("back-with-shirt")
      ? results["back-with-shirt"]?.url ?? null
      : null,
...
function logUploadPlanTable(context: string, entries: UploadPlanEntry[]) {
  ...
  try {
    const rows = entries.map((entry, index) => ({
      ...
      FrontWith:
        entry.uploads.frontWithShirt[0] ??
        entry.candidates.frontWithShirt[0] ??
        "—",
      FrontDesignOnly:
        entry.uploads.frontWithoutShirt[0] ??
        entry.candidates.frontWithoutShirt[0] ??
        "—",
      BackWith:
        entry.uploads.backWithShirt[0] ??
        entry.candidates.backWithShirt[0] ??
        "—",
      BackDesignOnly:
        entry.uploads.backWithoutShirt[0] ??
        entry.candidates.backWithoutShirt[0] ??
        "—",
...
function createDesignPayloadSnapshot(): Array<{
  ...
  return rawItemsSnapshot
    .map((item) => {
      ...
      const elementEntries = buildDesignElements(
        item.designState ?? null,
...
function logDesignPayloadSnapshot(context: string) {
  ...
  try {
    ...
    console.warn(
      `[Checkout] Failed to log design payload preview (${context})`,
      error
    );
  }
}
...
function buildDesignElements(
  ...
  for (const [viewName, view] of Object.entries(designState.views)) {
    ...
    for (const image of images) {
      const cacheRef =
        typeof (image as any)?.assetCacheRef === "string"
          ? (image as any).assetCacheRef.trim()
          : "";
      const originalSrc =
        typeof (image as any)?.originalSource === "string"
          ? (image as any).originalSource.trim()
          : "";
      const rawSrc =
        typeof image.imgUrl === "string" ? image.imgUrl.trim() : "";
      let resolvedSource: string | null = null;
      const lookupOrder = [cacheRef, originalSrc, rawSrc].filter(
...
async function recordOrderIfNeeded() {
  ...
  const resolveOrderItemId = (item: CartItem): string => {
    ...
    do {
      candidate = Math.floor(Math.random() * 100000000)
        .toString()
        .padStart(8, "0");
  ...
  const registerPreviewAsset = (params: {
    cartItemId: string;
    url: string | null;
    bucket: string | null;
  }) => {
  ...
  for (const [index, planEntry] of uploadPlanEntries.entries()) {
    ...
    const itemSlug = sanitizePathSegment(
      item?.id ?? `item-${index + 1}`,
      `item-${index + 1}`
    );

    const uploadRequests = buildPreviewUploadRequests(
      orderToken,
      itemSlug,
      planEntry,
      orderItemId
    ...
    for (const [sourceIndex, sourceInfo] of uploadedSources.entries()) {
      const uploadSource =
        sourceInfo.cacheRef ?? sourceInfo.original ?? sourceInfo.raw;
      if (!uploadSource) continue;
      const assetKeyForName =
        sourceInfo.original ?? sourceInfo.raw ?? uploadSource;
      ...
      if (totalDesignUploads > 0) {
        ...
        console.info(
          `[Checkout] Uploading design element ${
            sourceIndex + 1
          } of ${totalDesignUploads}`,
          {
            cartItemId: item.id,
            name: designName,
          }
      ...
      if (
        ...
        !finalUrl.startsWith("cache://")
      ) {
        const stored = Boolean(
          uploadedDesign && uploadedDesign !== uploadSource
        );
        designAssets.push({
          cartItemId: orderItemId,
          url: finalUrl,
    ...
    const canvasPreviewSources = item.canvasPreviews ?? {
      Front: null,
      Back: null,
    };
    const canvasPreviewCacheRefs = item.canvasPreviewCacheRefs ?? {
      Front: null,
      Back: null,
    };
    const blankPreviewSources = item.blankPreviews ?? {
      Front: null,
      Back: null,
    };
    ...
    for (const view of DESIGN_VIEWS) {
      ...
      const canvasStored =
        !uploadedCanvas.startsWith("data:") &&
  ...
  console.info("[Checkout] Design payload summary", {
    ...
    elements: designEntries.reduce(
      (total, entry) => total + (entry.elements?.length ?? 0),
      0
    ),
  });
  ...
  const email =
    typeof customerSnapshot.email === "string" && customerSnapshot.email.trim()
      ? customerSnapshot.email.trim()
      : "unknown@example.com";
  const phone =
    typeof customerSnapshot.phone === "string" && customerSnapshot.phone.trim()
      ? customerSnapshot.phone.trim()
      : "Not provided";
  const company =
    typeof customerSnapshot.company === "string" &&
    customerSnapshot.company.trim()
      ? customerSnapshot.company.trim()
      : null;
  const orderDetails =
    typeof customerSnapshot.notes === "string" && customerSnapshot.notes.trim()
  ...
  try {
    ...
    console.log(
      "[Checkout] Order function payload (uploads only, no external links)",
      orderPayload
    );

    if (!ORDER_FUNCTION_URL) {
      throw new Error(
        "Order function URL is not configured. Set VITE_ORDER_FUNCTION_URL."
      );
    }
    ...
    if (typeof window !== "undefined") {
      ...
      if (typeof generator === "function") {
        try {
          ...
          console.warn(
            "[Checkout] Failed to generate high‑res previews before order creation",
            error
          );
        }
      }
...
const requestStatus = ref<
  "idle" | "processing" | "success" | "canceled" | "error"
>("idle");
...
const canGoBack = computed(() => {
  ...
  if (currentStep.value === 3)
    return (
      requestStatus.value !== "processing" && requestStatus.value !== "success"
    );
  return false;
});
...
watch(
  ...
  (items) => {
    ...
    const current =
      items.find((item) => item.id === activeCartItemId.value) ?? items[0];
...
const activeCartItem = computed<CartItem | null>(() => {
  ...
  const existing = cartItems.value.find(
    (item) => item.id === activeCartItemId.value
  );
...
function resolvePreviewSources(
  item: CartItem | null
): Record<PreviewView, string | null> {
  if (!item) {
  ...
  const frontFallback =
    normalizePreview(item.previewImage) ??
    normalizePreview(color?.frontUrl) ??
    normalizePreview(color?.sideUrl);
  const backFallback =
    normalizePreview(color?.backUrl) ??
    normalizePreview(color?.frontUrl) ??
    normalizePreview(color?.sideUrl) ??
    frontFallback;
  return {
...
function resolveBlankPreviewSources(
  item: CartItem | null
): Record<PreviewView, string | null> {
  if (!item) {
    return { Front: null, Back: null };
...
const editingActiveItem = computed(() =>
  Boolean(
    checkoutStore.isEditingCartItem &&
      checkoutStore.editingCartItemId === activeCartItemId.value
  )
);
...
const previewSources = computed(() => {
  ...
  if (!item)
    return { Front: null, Back: null } as Record<PreviewView, string | null>;
  ...
  // Not actively editing? Just use the item-based resolver.
  if (!editingActiveItem.value) {
    return resolvePreviewSources(item);
  }

  // When editing: prefer the CART ITEM’s saved previews, then fall back to live checkoutStore.
  const live = (checkoutStore.designPreviews ?? {
    Front: null,
    Back: null,
  }) as Record<PreviewView, string | null>;
  ...
  return {
    Front:
      normalizePreview(item.designPreviews?.Front) ??
      normalizePreview(live.Front) ??
      frontFallback ??
      null,
    Back:
      normalizePreview(item.designPreviews?.Back) ??
      normalizePreview(live.Back) ??
      backFallback ??
      null,
  };
});
...
const blankPreviewSources = computed(() => {
  ...
  const itemBlanks = (item.blankPreviews ?? {
    Front: null,
    Back: null,
  }) as Record<PreviewView, string | null>;
  const itemCanvas = (item.canvasPreviews ?? {
    Front: null,
    Back: null,
  }) as Record<PreviewView, string | null>;
  ...
  const liveBlank = (checkoutStore.blankDesignPreviews ?? {
    Front: null,
    Back: null,
  }) as Record<PreviewView, string | null>;
  const liveCanvas = (checkoutStore.canvasPreviews ?? {
    Front: null,
    Back: null,
  }) as Record<PreviewView, string | null>;
  ...
  return {
    Front:
      normalizePreview(itemBlanks.Front) ??
      normalizePreview(itemCanvas.Front) ??
      normalizePreview(liveBlank.Front) ??
      normalizePreview(liveCanvas.Front) ??
      null,
    Back:
      normalizePreview(itemBlanks.Back) ??
      normalizePreview(itemCanvas.Back) ??
...
const hasDesignOnlyAny = computed(() =>
  Boolean(blankPreviewSources.value.Front || blankPreviewSources.value.Back)
);
  ...
  async (value) => {
    ...
    if (isCachedAssetRef(normalized)) {
      try {
        ...
        console.warn(
          "[Checkout] Failed to resolve cached preview for display",
          normalized,
          error
        );
        setLocalPreviewObjectUrl(null);
    ...
    if (normalized.startsWith("data:")) {
      try {
        ...
        console.warn(
          "[Checkout] Failed to convert data URL preview to blob",
          error
        );
        setLocalPreviewObjectUrl(null);
      }
    ...
    if (normalized.startsWith("blob:")) {
      try {
        ...
        console.warn(
          "[Checkout] Failed to resolve blob preview for display",
          normalized,
          error
        );
      }
...
async function submit() {
  ...
  const pricedItems = cartItems.value.filter(
    (item) => Number.isFinite(item.unitPrice) && item.unitPrice !== null
  );
  ...
  if (!CHECKOUT_FUNCTION_URL) {
    if (!SUPABASE_URL) {
      checkoutError.value =
        "Checkout service is not configured. Missing VITE_SUPABASE_URL.";
      return;
    }
    if (!SUPABASE_ANON_KEY) {
      checkoutError.value =
        "Checkout service is not configured. Missing VITE_SUPABASE_ANON_KEY.";
      return;
    }
  }
  if (!stripeConfigured.value) {
    checkoutError.value =
      "Payment service is not configured. Missing Stripe publishable key.";
    return;
  }
  ...
  try {
    ...
    if (!data?.clientSecret) {
      throw new Error(
        "Payment could not be initialised. Missing client secret."
      );
    }
...
const paymentFormReady = computed(
  () => paymentElementReady.value && Boolean(paymentIntentClientSecret.value)
);
...
.checkout-feature {
  ...
  background: linear-gradient(
    135deg,
    rgba(236, 252, 203, 0.85),
    rgba(196, 240, 194, 0.7)
  );
