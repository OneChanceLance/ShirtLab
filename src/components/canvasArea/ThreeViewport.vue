...
<script setup lang="ts">
  ...
  if ('colorSpace' in dynamicTexture) {
    (dynamicTexture as any).colorSpace = THREE.SRGBColorSpace
  } else if ('sRGBEncoding' in (THREE as any)) {
    (dynamicTexture as any).encoding = (THREE as any).sRGBEncoding
  }
        ...
        object.traverse((child) => {
          if ((child as THREE.Mesh).isMesh) {
            ...
            if (Array.isArray(mat)) {
              mat.forEach((m) => applyTexture(m))
            } else if (mat) {
              applyTexture(mat as THREE.Material)
            }
          }
        })
