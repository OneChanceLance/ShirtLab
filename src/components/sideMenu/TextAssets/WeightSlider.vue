<template>
    <div class="weight-slider">
        <div class="track-wrap" @click.stop>
            <input type="range" class="slider" v-model="inner" :min="min" :max="max" :step="step"
                :style="{ '--pct': pct + '%' }" @input="emitUpdate" list="steplist" />
            <datalist id="steplist">
                <option v-for="n in (max - min + 1)" :key="n" :value="min + (n - 1) * step" />
            </datalist>
            <div class="ticks">
