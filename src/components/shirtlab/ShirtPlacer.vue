    ...
    <div class="sidebar">
      <div class="viewport-frame">
        <ViewPort label="Front" :is-active="selectedView === 'Front'"
          :preview-src="frontPreview || viewToSrc.Front || '/tshirt.png'" @select="handleViewSelect" />
        <ViewPort label="Back" :is-active="selectedView === 'Back'"
          :preview-src="backPreview || viewToSrc.Back || viewToSrc.Front || '/tshirt.png'" @select="handleViewSelect" />
      </div>
...
<script setup lang="ts">
  ...
  function switchView(next: View, options: { skipStore?: boolean; force?: boolean } = {}) {
    ...
    setShirtBackground(viewToSrc[next] || viewToSrc.Front || '/tshirt.png');
    requestAutoGrid(false);
  }

  function handleViewSelect(label: string) {
    const view = label === 'Back' ? 'Back' : 'Front';
    switchView(view);
  }
  ...
  function handleClothingSelect(details: any) {
    ...
    if (selectedView.value !== 'Front') {
      ...
      loadViewState('Front');
      setShirtBackground(viewToSrc.Front || '/tshirt.png');
    }
  }
  ...
  watch(() => props.clothing, (c) => {
    ...
    const backgroundChange = Boolean(c.front || c.back || c.colors?.[0]?.background);
    ...
    try {
      ...
      if (source) {
        try {
          ...
          if (view === 'Front') frontPreview.value = url; else backPreview.value = url;
          return;
        } catch (directErr) {
  ...
  function drawRulers(ctx: CanvasRenderingContext2D, gridX: number, gridY: number, gridWidth: number, gridHeight: number) {
    ...
    for (let offset = 0; offset <= maxX; offset += quarterStep) {
      ...
      if (isInch && offset > 0) {
        ...
        const label = Number.isInteger(inches) ? inches.toString() : inches.toFixed(1);
  ...
  function updateClothing(details: any) {
    ...
    const backgroundChange = Boolean(details.front || details.back || details.colors?.[0]?.background);
    ...
    const assignableKeys: Array<keyof typeof clothingDetails.value> = ['name', 'image', 'style', 'gender', 'size'];
