<template>
  <div class="page" @click="hideContextMenu">
    <div class="lab-container" style="position: relative" ref="labContainer">
      ...
      />
      <div class="design-inspector" @click.stop>
        <button
          type="button"
          class="design-inspector__toggle"
          @click.stop="inspectorVisible = !inspectorVisible"
        >
          {{ inspectorVisible ? "Hide sizing" : "Show sizing" }}
        </button>
        <div v-if="inspectorVisible" class="design-inspector__panel">
          <div class="design-inspector__header">
            <div>
              <div class="design-inspector__title">
                Design sizing — {{ selectedView }}
              </div>
              <span class="design-inspector__subtitle">
                Bounds and coverage for pricing
              </span>
            </div>
          </div>

          <div class="design-inspector__metrics" v-if="activeInspector">
            <div class="design-inspector__metric">
              <div class="design-inspector__metric-label">Design area</div>
              <div>
                <span>
                  {{
                    activeInspector.summary.bounds.widthInches !== null &&
                    activeInspector.summary.bounds.heightInches !== null
                      ? `${activeInspector.summary.bounds.widthInches.toFixed(
                          2
                        )}″ × ${activeInspector.summary.bounds.heightInches.toFixed(
                          2
                        )}″`
                      : "—"
                  }}
              ...
              <div>
                <span>
                  {{
                    activeInspector.summary.bounds.areaSquareInches !== null
                      ? `${activeInspector.summary.bounds.areaSquareInches.toFixed(
                          1
                        )} sq in`
                      : ""
                  }}
            ...
            <div class="design-inspector__metric">
              <div class="design-inspector__metric-label">Grid</div>
              <div>
                <span>
                  {{
                    activeInspector.summary.grid.widthInches !== null &&
                    activeInspector.summary.grid.heightInches !== null
                      ? `${activeInspector.summary.grid.widthInches.toFixed(
                          2
                        )}″ × ${activeInspector.summary.grid.heightInches.toFixed(
                          2
                        )}″`
                      : "—"
                  }}
              ...
              <div>
                <span>
                  {{
                    activeInspector.summary.grid.areaSquareInches !== null
                      ? `${activeInspector.summary.grid.areaSquareInches.toFixed(
                          1
                        )} sq in`
                      : ""
                  }}
            ...
            <div class="design-inspector__metric">
              <div class="design-inspector__metric-label">Coverage</div>
              <div>
                <span>
                  {{
                    activeInspector.summary.coverageRatio !== null
                      ? `${Math.round(
                          activeInspector.summary.coverageRatio * 100
                        )}% of grid`
                      : "—"
                  }}
                </span>
              ...
              <div>
                <span>
                  {{
                    activeInspector.summary.sumAreaSquareInches !== null
                      ? `${activeInspector.summary.sumAreaSquareInches.toFixed(
                          1
                        )} sq in total`
                      : ""
                  }}
            ...
            <div class="design-inspector__metric">
              <div class="design-inspector__metric-label">Elements</div>
              <div>
                <span>
                  {{ activeInspector.summary.elementsCount }} object{{
                    activeInspector.summary.elementsCount === 1 ? "" : "s"
                  }}
                </span>
              </div>
            </div>
          ...
          <ul class="design-inspector__list" v-if="activeInspector">
            <li
              v-for="item in activeInspector.items"
              :key="item.id"
              class="design-inspector__row"
            >
              <div class="design-inspector__item-info">
                <div class="design-inspector__item-title">
                  {{ item.name }}
                </div>
                <div class="design-inspector__item-meta">
                  <span>
                    {{
                      item.widthInches !== null &&
                      item.heightInches !== null
                        ? `${item.widthInches.toFixed(
                            2
                          )}″ × ${item.heightInches.toFixed(2)}″`
                        : "—"
                    }}
                  </span>
                  <span v-if="item.areaSquareInches !== null">
                    · {{ item.areaSquareInches.toFixed(1) }} sq in
                  </span>
                </div>
              </div>
              <div class="design-inspector__item-position">
                x: {{ Math.round(item.position.x) }}, y:
                {{ Math.round(item.position.y) }}
              </div>
            </li>
            <li
              v-if="!activeInspector.items.length"
              class="design-inspector__empty"
            >
              No design elements on this side yet.
            </li>
          </ul>
        </div>
      </div>
...
function draw() {
  ...
  // Keep checkout pricing in sync with the exact sizing UI numbers.
  checkoutStore.setDesignMetrics(selectedView.value, {
    areaSquareInches:
      inspectorItems.length && Number.isFinite(boundingAreaInches)
        ? (boundingAreaInches as number)
        : null,
    coverageRatio:
      typeof coverageRatio === "number" && Number.isFinite(coverageRatio)
        ? coverageRatio
        : null,
  });
